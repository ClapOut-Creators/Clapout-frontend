    ...
    @if (leaderboard().entries.length) {
      ...
      >
        @for (entry of leaderboard().entries; track entry.creatorId) {
          <li
            class="flex min-h-[62px] items-center justify-between gap-4 rounded-[18px] border border-[#F0F0F0] bg-white px-[26px] py-[15px] lg:min-h-[76px]"
          >
            ...
            <span class="flex shrink-0 items-center gap-[11px]">
              <svg
                data-p-icon="eye"
                [size]="18"
              ...
              ></svg>
              <span
                class="text-[16px] leading-[23.8px] font-medium text-[#454545] [font-family:var(--clapout-font-heading)] lg:text-[18px]"
      ...
      @if (needsOwnStandingFooter(leaderboard())) {
        <p class="m-0 mt-[11px] text-[14px] leading-6 text-[#7B7B7B] lg:text-[16px]">
          ...
          {{ formatViews(leaderboard().me?.verifiedViews ?? 0) }} views.
        </p>
      }
    } @else {
